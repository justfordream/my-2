...
import com.huateng.crm.common.AppCode;
...
public class SendMessageUtil implements IClientSendMessage{
	 ...
	 * @return
	 */
	public Message sendMsg(final String reqTxnSeq, final String orgId, final String context, final String txnCd, 
...
//			LOG.error("InterruptedException is {}", e);
		...
		if (acqureresult && tempresult != null) {
			...
			LOG.error("=============receive message time out!reqTxnSeq:{}",
					reqTxnSeq);
			LOG.error("=============超时未收到消息!reqTxnSeq:{}, map size is [{}]", reqTxnSeq , excuterthreadsmap.size());
			throw new ServiceException("UPAY-C-5A07");
			//return null;
		}
	...
	public String sendTextRtnMessage(final String seq, final String orgId, final String context, final String txnCd, 
			 final Long hNum ) throws Exception {
		LOG.info("CRM前置发给核心平台组装后报文信息:[{}]",new Object[]{DateUtil.paseLog(context)});
		LOG.debug("CRM前置发给核心平台组装后报文信息:[{}]",new Object[]{context});
		log.info("CRM前置发给核心平台组装后报文信息");	
		...
		try {
			...
			//LOG.info(resp);
			LOG.info("省前置接收核心返回的响应信息为: {}",new Object[]{resp});
			log.info("省前置接收核心返回的响应信息");
