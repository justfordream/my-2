...
import com.huateng.core.remoting.CrmTiCatRemoting;
 ...
 */
public class CrmTianCatRemotingImpl implements CrmTiCatRemoting {
	...
	public String sendMsg(String header, String... args) {
		...
		try {
			if (StringUtils.isNotBlank(args[0])&&StringUtils.isNotBlank(args[1])) {
				 ...
				 * 根据路由信息发送数据
				 */
				logger.info("......根据路由信息向CRM[{}]发送信息......",new Object[]{routeMsg.getReqIp()});
				logger.info("......assumly xmlhead is :{}",new Object[]{xmlHeader});
				logger.info("......assumly xmlbody is :{}",new Object[]{xmlBody});
				/*
				 * 全网监控日志
				 */
//				this.logHandle.info(true, CoreConstant.ErrorCode.SUCCESS.getCode(), reqBoss.getTransInfo().getTransIDO(),
//						reqBoss.getBIPType().getActivityCode(), UpayCommon.getTransDesc(reqBoss.getBIPType().getActivityCode()), 
...
//				reqBoss.setSvcCont(encryptBodyText);
				...
				logger.info("......接收到CRM:[{}]响应信息:{}",new Object[]{routeMsg.getReqIp(),responseText});
				/*
				 * 解密报文体
				 */
...
//				rspBoss.setSvcCont(resDecryptBodyText);				
				 ...
				 * 返回信息
				 */
				logger.info(".......将CRM:[{}]响应信息发送天猫前置，信息如下:{}",new Object[]{routeMsg.getReqIp(),responseText});
