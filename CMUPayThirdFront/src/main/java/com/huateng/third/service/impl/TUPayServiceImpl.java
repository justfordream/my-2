 ...
 * 
 */
public class TUPayServiceImpl extends BaseServiceImpl implements TUPayService {
	...
	public Map<String, String> checkSign(String client,
		...
		if (CoreConstant.SWITCH_OPEN.equalsIgnoreCase(checkSwitch)) {
			...
			this.logHandle.info(false,
					...
					paramsMaps.get(UnionPayMsg.ORDERID.getValue()),
					paramsMaps.get(UnionPayMsg.TXNTYPE.getValue()), "",
					CoreConstant.UNION_PAY_ORG_ID,
					CoreConstant.UNION_PAY_ORG_ID,
			...
			boolean isValid = tupaySecurityRemoting.verify(
					CoreConstant.UNION_PAY_ORG_ID, paramsMaps);
			if (!isValid) {
	...
	public Map<String, String> sign(String client,
		...
		if (CoreConstant.SWITCH_OPEN.equalsIgnoreCase(signSwitch)) {
			...
			if (MapUtils.isNotEmpty(paramsMaps)) {
				try {
					afterParamsMaps = tupaySecurityRemoting.sign(CoreConstant.UNION_PAY_ORG_ID, paramsMaps);
