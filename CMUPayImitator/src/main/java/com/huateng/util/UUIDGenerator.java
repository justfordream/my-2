...
public class UUIDGenerator {
	...
	/**
	 * yyyyMMddHHmmssSSS+UUID字符串
	 * 
	 * @return
	 */
	public static String getDateAndUUID(int length) {
		UUID uuid = UUID.randomUUID();
		String str = uuid.toString();
		// 去掉"-"符号
		String temp = str.replaceAll("-", "");
		String detailDate = DateUtil.getDateyyyyMMddHHmmssSSS();
		String result = detailDate+ temp;
	...
	public static String randomNum(int length){
		...
		long random = Math.abs(rdm.nextLong());
		String result = detailDate.substring(0, 17)+Long.toString(random);
		return result.substring(0, length);
	}

	public static void main(String[] args) {
//		System.out.println(UUIDGenerator.generateUUID());
		System.out.println(UUIDGenerator.randomNum(2));
	}
