...
/**
 * @date 2013-12-02
 * @author chenjun 模拟省天猫功能：作为服务端，接收发送过来的信息，并把接收到的报文保存如文件
 *        
 */
public class ReceiveMsgAutoResponServletForTmall extends HttpServlet {
	...
	private final Logger log = Logger.getLogger(ReceiveMsgAutoResponServletForTmall.class);
	private static final XmlDomImp xmldom = new XmlDomImp();
	private static final TimeUtil tUtil = new TimeUtil();
	public ReceiveMsgAutoResponServletForTmall() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			pathName.append(GN.receivefilepath).append(File.separator).append("Tmall").append(File.separator).append(tUtil.getLocalDate());
			...
			respFilePath.append(GN.tmalltemprepfilepath).append(File.separator).append(respFileName).append(".xml");
			...
			log.debug("仿真天猫侧返回报文路径"+respFilePath);
			log.debug("仿真天猫侧返回报文"+responseMsg);
			//去掉格式
...
//				"</SubID>", IDGenerator.getSubIdForBank(reqkey), responseMsg);
			...
			if("open".equals(bankEncodeFlag)&&bankEncodeActivityCode.contains(activityCode)==true){
				log.debug(".........天猫端加密，报文为"+activityCode);
