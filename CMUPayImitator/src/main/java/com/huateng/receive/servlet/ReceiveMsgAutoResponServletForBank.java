...
/**
 * @date 2013-03-02
 * @author Administrator 功能：作为服务端，接收银行发送过来的信息，并保存文件
 */
public class ReceiveMsgAutoResponServletForBank extends HttpServlet {
	...
	private final Logger log = Logger.getLogger(ReceiveMsgAutoResponServletForBank.class);
	private static final TimeUtil tUtil = new TimeUtil();
	private static final XmlDomImp xmldom = new XmlDomImp();
	public ReceiveMsgAutoResponServletForBank() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			pathName.append(GN.receivefilepath).append(File.separator).append("bank").append(File.separator).append(tUtil.getLocalDate());
			...
			respFileName = xmldom.getgrdsonNdVal_String(reqMsg, "Header","ActivityCode");
			...
			routeType =  XmlStringUtil.parseNodeValueFromXml("<RouteType>", "</RouteType>", reqMsg);
			routeValue =  XmlStringUtil.parseNodeValueFromXml("<RouteValue>", "</RouteValue>", reqMsg);
			homeDomain =  XmlStringUtil.parseNodeValueFromXml("<HomeDomain>", "</HomeDomain>", reqMsg);
			...
			respFilePath.append(GN.banktemprepfilepath).append(File.separator).append(respFileName).append(".xml");
			...
			log.debug("仿真银行侧返回报文路径"+respFilePath);
			log.debug("仿真银行侧返回报文"+responseMsg);
			//去掉格式
			...
			responseMsg = XmlStringUtil.relaceNodeContent("<RouteType>",
					"</RouteType>", DateUtil.getDateyyyyMMddHHmmss(), responseMsg);
			responseMsg = XmlStringUtil.relaceNodeContent("<RouteValue>",
					"</RouteValue>", DateUtil.getDateyyyyMMddHHmmss(), responseMsg);
			responseMsg = XmlStringUtil.relaceNodeContent("<HomeDomain>",
					"</HomeDomain>", DateUtil.getDateyyyyMMddHHmmss(), responseMsg);
			/**
			 * @time 20131114
			 * */
...
//				"</SubID>", IDGenerator.getSubIdForBank(reqkey), responseMsg);
			...
			if("open".equals(bankEncodeFlag)&&bankEncodeActivityCode.contains(activityCode)==true){
				log.debug(".........银行端加密，报文为"+activityCode);
