...
import com.huateng.cmupay.parseMsg.reflect.vo.bank.MobileShopGetMsgReqVo;
import com.huateng.cmupay.parseMsg.reflect.vo.bank.MobileShopGetMsgResVo;
...
@Controller("mmarketTPayGetMsgAction")
@Scope("prototype")
public class RcvShopGetMsgAction extends AbsBaseAction<MobileShopMsgVo, MobileShopMsgVo>{
	...
	private @Value("${GETMSG_PATH}") String getMsgPath;
	...
	public MobileShopMsgVo execute(MobileShopMsgVo msgVo)
		...
		MobileShopGetMsgReqVo reqBody = new MobileShopGetMsgReqVo();
		MobileShopGetMsgResVo resBody = new MobileShopGetMsgResVo();
		String transIDH="";
...
//			String oprId=Serial.genSerialNum(CommonConstant.Sequence.OprId.toString());
			...
			if (checkFlag == null) {
				...
				//请求值
				toUnion(paramMap,reqBody,reqMsg);
				//vo设值   
				tpayMsgVo.setTpayReqData(paramMap);
				tpayMsgVo.setTransIDO(transIDH);
				tpayMsgVo.setReqPathAppend(getMsgPath);
				...
				//入库更新
				toTxnLog(resultMap,tTxnLog,year,mBankAcctID);
				//map转化为vo
				toResMsg(resMsg,reqMsg,resBody,reqBody,resultMap,transIDHTime,mBankAcctID);
				//待确认
...
//				tTxnLog.setRcvCnlType(oriTxnLog.getRcvCnlType());
				...
				if (TUPayConstant.UnPayRspCode.UNPAY_00.getValue().equals(
					...
					marketLogger.info("银联响应成功!内部交易流水:{},发起方:{},接收方:{}",
							new Object[] { transIDH,reqMsg.getReqSys(),forwardOrg});
...
//					resBody.setOriResultDesc(RspCodeConstant.Bank.getDescByValue(errNameBody));
	...
	private void initLog(TpayCsysTxnLog tTxnLog, Long seqId, String transIDH,
			...
			UpayCsysTransCode transCode, MobileShopMsgVo resMsg,MobileShopGetMsgReqVo reqBody ) {
...
//		tTxnLog.setIntMqSeq(reqMsg.getMqSeq());
	 ...
	 * 发往支付前置的值
	 * */
	private void toUnion(Map<String,String> paramMap,MobileShopGetMsgReqVo reqBody,MobileShopMsgVo reqMsg){
//		版本号	
...
//		交易类型	
		paramMap.put("txnType",TUPayConstant.TpayType.TPAYTYPE_77.getValue());
//		交易子类	*
...
//		商户代码	
		TpayCsysMerCodeInfo tpayCsysMerCodeInfo=MerCodeCache.getMerInfoByOrgCode(reqMsg.getReqSys(),CommonConstant.BankOrgCode.TPAY.getValue(),reqBody.getShopMerID());
		if(tpayCsysMerCodeInfo!=null){
			paramMap.put("merId",tpayCsysMerCodeInfo.getOrgCode());
			paramMap.put("merType",tpayCsysMerCodeInfo.getMerType());
			paramMap.put("merName",tpayCsysMerCodeInfo.getMerName());
		}
//		商户简称
		paramMap.put("merAbbr",reqBody.getMerAbbr());
//		短信类型
		paramMap.put("smsType",reqBody.getSmsType());
//		帐号	
		paramMap.put("accNo",reqBody.getBankAcctID());
//		商户订单号	orderId
		paramMap.put("orderId",reqBody.getOrderID());
//		订单发送时间	txnTime
		paramMap.put("txnTime",reqBody.getTxnTime());
//		银行卡验证信息及身份信息	
	 ...
	 * 返回的需要入库的值
	 * */
	private void toTxnLog(Map<String,String>paramMap,TpayCsysTxnLog tTxnLog,int year,String mBankAcctID){
	 ...
	 * 报文返回值设置
	 * */
	private void toResMsg(MobileShopMsgVo resMsg,MobileShopMsgVo reqMsg,MobileShopGetMsgResVo resBody,MobileShopGetMsgReqVo reqBody,Map<String,String> map,String transIDHTime,String bankAcctID){
		//报文头
		resMsg.setActivityCode(reqMsg.getActivityCode());
		resMsg.setReqSys(reqMsg.getReqSys());
		resMsg.setReqChannel(reqMsg.getReqChannel());
		...
		resBody.setBankAcctID(bankAcctID);
