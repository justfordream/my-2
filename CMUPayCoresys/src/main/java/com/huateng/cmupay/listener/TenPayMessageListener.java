 ...
 * 
 */
public class TenPayMessageListener extends AbsMessageListener {
	...
	public void onMessage(Message message) {
		try {
			...
			if (message instanceof ObjectMessage) {
				...
				// 生成一个新的交易流水号
				String intTxnSeq = Serial
						.genSerialNo(CommonConstant.Sequence.IntSeq.toString());
				...
				logger.info("接收到支付前置发来的财付通报文:{}, 内部交易流水：{}", new Object[] {
						(obj == null ? "报文内容为空！" : obj), intTxnSeq });
				log.info("接收到支付前置发来的财付通报文:{}, 内部交易流水：{}", new Object[] {
						(obj == null ? "报文内容为空！" : obj), intTxnSeq });

				if (!(obj instanceof Map<?, ?>)) {
					logger.info(
							"支付前置发来的财付通报文内容格式不是Map<String, String>:{}, 内部交易流水：{}",
							new Object[] { (obj == null ? "报文内容为空！" : obj),
									intTxnSeq });
					log.info(
							"支付前置发来的财付通报文内容格式不是Map<String, String>:{}, 内部交易流水：{}",
							new Object[] { (obj == null ? "报文内容为空！" : obj),
				...
				// 解析报文，取得外部业务功能码,交易码
				String reqActivityCode = (String) messageMap.get("#TXN_CODE"); // 获取交易码
				String orderId = (String) messageMap.get(TenPayMsg.OUT_TRADE_NO
						.getValue());
				String merId = (String) messageMap.get(TenPayMsg.PARTNER
						.getValue());
				String txnTime = (String) messageMap.get(TenPayMsg.TIME_END
						.getValue());
				...
				try {
					...
					queryMap.put("reqActivityCode", thridOrgId
							+ reqActivityCode);
					upayCsysTransCode = transCodeConvert(queryMap);
					if (upayCsysTransCode == null
							|| StringUtils.isBlank(upayCsysTransCode
									.getTransCode())) {
						logger.warn(
								"发起方机构没有此服务的权限，内部交易流水:{},订单号:{},商户ID:{},订单发送时间:{},交易类型:{}",
								new Object[] { intTxnSeq, orderId, merId,
										txnTime, reqActivityCode });
						log.warn(
								"发起方机构没有此服务的权限，内部交易流水:{},订单号:{},商户ID:{},订单发送时间:{},交易类型:{}",
								new Object[] { intTxnSeq, orderId, merId,
										txnTime, reqActivityCode });
					...
					logger.info(
							"收到支付前置发来的报文，内部交易流水:{},内部交易代码{},订单号:{},商户ID:{},订单发送时间:{},交易类型:{}",
							new Object[] { intTxnSeq, transCode, orderId,
									merId, txnTime, reqActivityCode });
					log.debug(
							"收到支付前置发来的报文，内部交易流水:{},内部交易代码{},订单号:{},商户ID:{},订单发送时间:{},交易类型:{}",
							new Object[] { intTxnSeq, transCode, orderId,
									merId, txnTime, reqActivityCode });
					...
					logger.error(
							"未知异常 {}，内部交易流水:{},内部交易代码{},订单号:{},商户ID:{},订单发送时间:{},交易类型:{}",
							new Object[] { e, intTxnSeq, orderId, merId,
									txnTime, reqActivityCode });
					log.error(
							"未知异常 {}，内部交易流水:{},内部交易代码{},订单号:{},商户ID:{},订单发送时间:{},交易类型:{}",
							new Object[] { e, intTxnSeq, orderId, merId,
