...
import com.huateng.cmupay.constant.TUPayConstant.ALIPayMsg;
 ...
 *
 */
public class ALIPayMessageListener extends AbsMessageListener {
	...
	public void onMessage(Message message) {
		try {
			...
			if (message instanceof ObjectMessage) {
				...
				// 解析报文，取得外部业务功能码,交易码
				final String reqActivityCode = ALIPayMsg.TXN_TYPE.getDesc(); // 报文的交易码  固定为01
	 			String orderId = (String) messageMap.get(ALIPayMsg.OUT_TRADE_NO.getValue());
				String txnTime = (String) messageMap.get(ALIPayMsg.NOTIFY_TIME.getValue());
...
//					if (isTrue) {
						...
						if (upayCsysTransCode == null || StringUtils.isBlank(upayCsysTransCode
										.getTransCode())) {
							logger.warn(
									"发起方机构没有此服务的权限，内部交易流水:{},订单号:{},订单发送时间:{},交易类型:{}",
									new Object[] { intTxnSeq, orderId, txnTime, reqActivityCode});
							log.warn(
									"发起方机构没有此服务的权限，内部交易流水:{},订单号:{},订单发送时间:{},交易类型:{}",
									new Object[] { intTxnSeq, orderId, txnTime, reqActivityCode });
						...
						logger.info("收到支付前置发来的报文，内部交易流水:{},内部交易代码{},订单号:{},订单发送时间:{},交易类型:{}",
								new Object[] { intTxnSeq, transCode, orderId, txnTime, reqActivityCode });
						log.debug("收到支付前置发来的报文，内部交易流水:{},内部交易代码{},订单号:{},订单发送时间:{},交易类型:{}",
								new Object[] { intTxnSeq, transCode, orderId, txnTime, reqActivityCode });
					...
					logger.error("未知异常 {}，内部交易流水:{},内部交易代码{},订单号:{},订单发送时间:{},交易类型:{}" , 
							new Object[]{e, intTxnSeq, orderId, txnTime, reqActivityCode });
					log.error("未知异常 {}，内部交易流水:{},内部交易代码{},订单号:{},订单发送时间:{},交易类型:{}" , 
							new Object[]{e, intTxnSeq, orderId, txnTime, reqActivityCode});
				}
