 ...
 *
 */
public class TmallQueryMessageListener extends AbsMessageListener{
		...
		try {
			...
			if (message instanceof TextMessage) {
				...
				if (replyToDest != null) {
					...
					if(RspCodeConstant.Bank.BANK_010A00.getValue().equals(msgVo.getRspCode())){
						tmallOperLogger.succ(
								"天猫交易成功,发起流水ReqTransID:{},应答流水RcvTransID:{},内部交易流水:{},应答码:{},天猫机构号:{},内部交易码:{}",
								new Object[] { msgVo.getReqTransID(), msgVo.getRcvTransID(),
										msgVo.getTxnSeq(), msgVo.getRspCode(),msgVo.getReqSys(),transCode });
					} else {
						tmallOperLogger.error(
								"天猫交易失败，发起流水ReqTransID:{},应答流水RcvTransID:{},内部交易流水:{},应答码:{},天猫机构号:{},内部交易码:{}",
								new Object[] { msgVo.getReqTransID(),msgVo.getRcvTransID(),
										msgVo.getTxnSeq(), msgVo.getRspCode(),msgVo.getReqSys(),transCode});
					}
					logger.info(
							"天猫交易完成,发起流水ReqTransID:{},应答流水RcvTransID:{},内部交易流水:{},应答码:{},天猫机构号:{},内部交易码:{}",
							new Object[] { msgVo.getReqTransID(), msgVo.getRcvTransID(),
									msgVo.getTxnSeq(), msgVo.getRspCode(),msgVo.getReqSys(),transCode });
				}
