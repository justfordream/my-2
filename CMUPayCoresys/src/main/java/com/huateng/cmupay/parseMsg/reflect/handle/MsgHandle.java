...
public class MsgHandle {
	...
	public static void unmarshaller(Object msg, String xmlDoc) {
		try {
			...
			if (xmlDoc == null || "null".equalsIgnoreCase(xmlDoc.trim())
					|| "".equals(xmlDoc.trim()) /*|| (Arrays.asList(ExcConstant.WZW_BODY_MSG_ERROR)).contains(xmlDoc.trim())*/) {
				return;
			}
			// 加载xml,生成Document
			xmlDoc = xmlDoc.trim();
			...
			log.error("类名:{}",msg.getClass().getName());
			throw new AppRTException("U99999", e.getMessage());
		}
	}
	...
	public static void unmarshallerNotCatch(Object msg, String xmlDoc) throws Exception {
			log.debug("解析xml,转化到响应对象msg的属性中:{}",new Object[]{msg.getClass().getName()});
			if (xmlDoc == null || "null".equalsIgnoreCase(xmlDoc.trim())
					|| "".equals(xmlDoc.trim()) /*|| (Arrays.asList(ExcConstant.WZW_BODY_MSG_ERROR)).contains(xmlDoc.trim())*/) {
				return;
	...
	private static void xml2Bean(String xmlDoc, Object msg, Field[] childFields)
		...
		// 用来记录在将XML转换成Obj时的数字格式化异常，即使发生该异常，也要让转换进程继续运行，结束时再抛出该异常。
						...
						if (field.getType().equals(Long.class)) {
							try {
								...
								if(numberFormatException == null) {
									numberFormatException = new NumberFormatException("字段" + setMethod.getName().replaceFirst("set", "") 
											+ "赋值错误！");
								}
								log.debug("MsgHandle#xml2Bean 字段" + setMethod.getName().replaceFirst("set", "") 
										+ "赋值错误！ 异常:" + e);
								continue;
							}
		...
		// 如果转换过程中有抛出过numberFormatException异常，则在转化结束后再抛出
		if(numberFormatException != null) {
