 ...
 * @param <R>
 */
public abstract class AbsSendBankJmsMessage<T, R> implements ISendJmsMessage {
	...
	protected UpayCsysRouteInfo findRouteInfo(Map<String, String> param)
		...
		logger.debug("  AbsSendBankJmsMessage findRouteInfo(Map<String,String>) - Start");
		...
		logger.debug("  AbsSendBankJmsMessage findRouteInfo(Map<String,String>) - end");
		return routeInfo;
	...
	public String sendMsg(JmsMsgHeader jmsMsgHeader, String jmsBody)
			throws AppBizException {
		log.info("->BANK:{}",new Object[]{jmsBody});
		logger.debug("AbsSendBankJmsMessage sendMsg(JmsMsgHeader, String) - start");
...
//					respDest.toString(), header.getMqSeq() + "_" + msg);
			...
			try {
				...
				logger.error("sendMsg(JmsMsgHeader, String)", e); 
				throw new AppBizException(RspCodeConstant.Upay.UPAY_U99999.getValue(), e);
			}

			logger.debug("AbsSendBankJmsMessage sendMsg(JmsMsgHeader, String) - end");
			return respStr;
		} else {
