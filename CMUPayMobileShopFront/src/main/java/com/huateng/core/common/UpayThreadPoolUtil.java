...
public class UpayThreadPoolUtil {
	...
	public static String putReqestQueue(Runnable runnable) throws Exception {
		...
		try {
			...
			logger.debug("任务加入线程池成功！当前线程数为:{}", executor.getLargestPoolSize());
		} catch (RejectedExecutionException e) {
			logger.error("任务加入线程池失败！当前线程数为:{}", executor.getLargestPoolSize());
	...
	public static int getCurrentThreadCount() throws Exception{
		...
		threadCount = executor.getLargestPoolSize();		
		return threadCount;
	}
